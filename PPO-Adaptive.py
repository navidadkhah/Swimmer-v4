...
class PPO_Agent:
    def __init__(self, env, target_kl, learning_rate, discount, entropy_coef, value_coef, ppo_epochs, batch_size):
        self.env = env
    ...
    def learn(self):
        ...
        for _ in range(self.ppo_epochs):
            for idx in range(0, len(states), self.batch_size):
                ...
                kl_divergence = (sampled_old_log_probs - new_log_probs).mean()

                if kl_divergence > 1.5 * self.target_kl:
                    break

                surr1 = ratio * sampled_advantages
                surr2 = torch.clamp(ratio, 1 - self.target_kl, 1 + self.target_kl) * sampled_advantages
...
class Model_TrainTest:
    def __init__(self, hyperparams):
        ...
        self.agent = PPO_Agent(env=self.env, target_kl=self.target_kl, learning_rate=self.learning_rate,
    ...
    def plot_training(self, episode):
        ...
        if episode == self.max_episodes:
            plt.savefig('./Adaptive/reward_plot.png', format='png', dpi=600, bbox_inches='tight')
        plt.tight_layout()
        plt.grid(True)
        plt.show()
        plt.clf()
...
if __name__ == '__main__':
    ...
    RL_hyperparams = {
        ...
        "RL_load_path": f'./Adaptive/final_weights' + '_' + '3000' + '.pth',
        "save_path": f'./Adaptive/final_weights',
        "save_interval": 500,
        "target_kl": 0.06,
        "learning_rate": 3e-4,
